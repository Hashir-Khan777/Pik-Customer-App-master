...
import DriverStatuses from '../../../pik-node-master/src/constants/DriverStatuses';
      ...
      .then(async (data) => {
        if (data.success) {
          ...
          await AsyncStorage.setItem(
            'device-info',
            JSON.stringify({fcmToken, ...deviceInfo}),
          );
        } else {
          console.log(data);
        }
...
const RootNavigatorComponent = ({
  ...
  const bootstrapRoutes = () => {
    if (!authInitialized) {
      return (
        <RootNavigator.Screen name="AppLoading" component={LoadingScreen} />
      );
    } else {
      if (!authLoggedIn) {
  ...
  React.useEffect(() => {
    if (authInitialized) SplashScreen.hide();
  }, [authInitialized]);
    ...
    const timer = setInterval(() => {
      getCurrentPosition()
        .then((info) => {
          ...
          if (!props.locationAvailable) props.setLocationAvailable(true);
        })
        .catch(() => {
          // console.log('GPS On/Off state [Off]')
          if (props.locationAvailable) props.setLocationAvailable(false);
        });
    }, 5000);
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setLocationAvailable: (available) =>
    dispatch(setLocationAvailableAction(available)),
