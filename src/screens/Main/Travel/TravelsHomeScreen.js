...
const Header = ({active, onTabSwitch}) => {
  ...
  const tabs = [
    t('pages.travel.tabs.travels'),
    t('pages.travel.tabs.started_by_me'),
    t('pages.travel.tabs.started_by_other'),
  ];
  return (
      ...
      {tabs.map((tab, index) => (
        <TouchableOpacity
          ...
          <Text
            style={[
              styles.tabItem,
              active === index ? styles.tabItemActive : null,
            ]}>
            {tab}
          </Text>
        </TouchableOpacity>
  ...
  const orderAddress = useMemo(() => {
    ...
    // i send the package
      ...
      switch (order.status) {
        ...
        case 'Pending':
        case 'Progress':
          return order?.delivery?.address?.formatted_address || defaultAddress;
        case 'Delivered':
          return order?.delivery?.address?.formatted_address;
        case 'Canceled':
          return order?.pickup?.address?.formatted_address;
        case 'Returned':
          return order?.pickup?.address?.formatted_address;
        case 'Reschedule':
          return order?.pickup?.address?.formatted_address;
  ...
  return (
    <View style={styles.orderContainer}>
      <Text style={styles.orderTitle}>
        {t('pages.travel.' + orderTitle.replace(/\s/g, ''))}
      </Text>
      <View
        style={[globalStyles.flexRowCenter, {justifyContent: 'space-between'}]}>
      ...
      <View style={globalStyles.flexRowCenter}>
        ...
        <Text style={[styles.orderStatus, styles[statusTitle]]}>
          {t(('pages.travel.status.' + statusTitle).replace(/\s/g, ''))}
        </Text>
      </View>
      {order.status === 'Progress' && (
        <View style={[globalStyles.flexRowCenter, {paddingTop: 16}]}>
          ...
          <View style={{flexGrow: 1}}>
            ...
            <Text style={styles.orderDriverTitle}>
              {t('pages.travel.vehicle.' + order.vehicleType)}
            </Text>
          </View>
          <GradientButton
            onPress={() =>
              navigation.navigate('MainTravelOrderTracking', {order})
            }
...
const TravelsHomeScreen = ({
  ...
  React.useEffect(() => {
    if (!authUser?.loggedIn) return;
    if (!ordersLoading) loadOrdersList();
  }, [authUser]);
  ...
  let filteredOrder = useMemo(() => {
    ...
    return orders
      .filter((o) => {
        ...
        if (isAvailableOrder(o, user)) return false;
        if (activeTab === 0) return true;
        if (activeTab === 1)
          return (
            (!o.isRequest &&
              (o.sender._id === user._id || o.sender === user._id)) ||
            (o.isRequest &&
              (o.receiver._id === user._id || o.receiver === user._id))
          );
        return (
          (!o.isRequest &&
            (o.receiver._id === user._id || o.receiver === user._id)) ||
    ...
    <KeyboardAvoidingScreen>
      <PageContainerDark
        ...
        {ordersLoading && !refreshing && (
          <ActivityIndicator size="large" color={COLOR_PRIMARY_500} />
        )}
          ...
          if (order.sender != null && order.receiver != null) {
            return (
              <TouchableOpacity
                key={index}
                onPress={() =>
                  navigation.navigate('MainTravelOrderDetail', {
                    orderId: order._id,
                  })
                }>
                <OrderItem
